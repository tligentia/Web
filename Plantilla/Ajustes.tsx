...
import { X, Key, ShieldCheck, RefreshCcw, Eye, EyeOff, Lock, Database, Loader2, Sparkles, Send, Cpu, ChevronDown, Globe, Plus, Zap, AlertCircle } from 'lucide-react';
...
export const Ajustes: React.FC<AjustesProps> = ({ isOpen, onClose, apiKey, onApiKeySave, userIp }) => {
  ...
  const handleAiAsk = async () => {
    ...
    try {
      ...
      setAiAnswer(`Error del Motor: ${e.message}`);
      if (e.message === "API_KEY_REQUIRED") setAiTip(true);
    } finally {
      setIsAsking(false);
    }
  };
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                <input 
                  ...
                  placeholder="AIzaSy..."
                  className={`w-full bg-white border p-3 rounded-2xl text-xs font-mono outline-none focus:ring-2 focus:ring-gray-900 transition-all shadow-sm ${aiTip ? 'border-red-500 ring-2 ring-red-100' : 'border-gray-200'}`}
                />
                <button 
                  type="button" 
              ...
              {aiTip && (
                <div className="flex items-center gap-2 text-[9px] font-bold text-red-700 uppercase tracking-widest px-1 animate-bounce">
                  <AlertCircle size={10} /> Introduce tu API Key arriba para activar la IA
                </div>
              )}
            </div>
